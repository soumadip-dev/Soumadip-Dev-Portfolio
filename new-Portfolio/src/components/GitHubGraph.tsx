  ...
  return (
    <section className="container py-12">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-medium text-muted-foreground">GitHub Contributions</h2>
        <a
              ...
              {contributions.map((week, weekIndex) => (
                <div key={weekIndex} className="flex flex-col gap-[3px]">
                  {week.days.map((day, dayIndex) => (
                    <button
                      key={dayIndex}
                      className={`w-2.5 h-2.5 md:w-[10px] md:h-[10px] rounded-[2px] ${getLevelColor(
                        day.level
                      )} transition-all hover:scale-110 hover:ring-1 hover:ring-primary/50 active:scale-105`}
