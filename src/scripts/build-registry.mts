// Thanks @ncdai & @shadcn for the inspiration and some code references in this file.
  ...
  const registryJSON = JSON.stringify(
    {
      ...
      items: registry.items
        ...
        .map((item) => {
          return {
            ...item,
            author: item.author ?? 'harshalvk <harshalvkhobragade@gmail.com>',
            files:
              item.files?.map((file) => {
...
async function applyIconTransform(content: string, filename: string) {
  ...
  await (transformIcons as unknown as (opts: IconTransformInput) => Promise<unknown>)({
    filename,
    raw: content,
    sourceFile,
    config,
